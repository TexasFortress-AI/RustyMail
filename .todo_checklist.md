...
- [ ] **Resolve Duplicate Types:**
  - [ ] Fix duplicate definition of `Arc` in `src/api/mcp_stdio.rs`. (Error E0252)
  - [ ] Fix duplicate definition of `AppState` in `src/api/rest.rs`. (Error E0255)
  - [ ] Fix duplicate definition of `StoreOperation` (`imap/session.rs` vs `imap/types.rs`). (Error E0255)
